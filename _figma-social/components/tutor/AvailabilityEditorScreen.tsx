...
export function AvailabilityEditorScreen({ onBack, onSave }: AvailabilityEditorScreenProps) {
  ...
  const handleSave = () => {
    ...
    if (selectedDates.length === 0 || !hasSlots) {
      alert('Please select at least one date and add at least one time slot');
      return;
    }
    
    // Convert to legacy format for compatibility
    const allSlots = dayAvailabilities.flatMap((da) =>
      da.slots.map((slot) => ({
        id: slot.id,
        time: slot.startTime,
  ...
  return (
    <div className="h-full bg-background flex flex-col">
      ...
      <div className="px-4 py-4 bg-background border-t border-border flex-shrink-0">
        <button
          onClick={handleSave}
          disabled={selectedDates.length === 0 || !dayAvailabilities.some(da => da.slots.length > 0)}
