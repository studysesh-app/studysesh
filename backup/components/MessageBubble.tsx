        ...
        <View className={`flex-row ${isMyMessage ? 'justify-end' : 'justify-start'} mb-1.5`}>
            <View className={`max-w-[75%] ${isMyMessage ? 'items-end' : 'items-start'}`} style={{ gap: 2 }}>
                {isMyMessage ? (
                    ...
                    <View className="px-4 py-2.5 rounded-[20px] rounded-bl-[4px] bg-[#e5e5ea] dark:bg-gray-800" style={{ borderWidth: 1, borderColor: 'rgba(0,0,0,0.04)' }}>
                        <Text className="text-[17px] text-black dark:text-white" style={{ lineHeight: 22 }}>
                            {message}
                        </Text>
                    </View>
                )}
